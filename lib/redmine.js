...
var proto = {
	...
	, request: function(method, path, params){
		...
		if (params.impersonate) {
			options.headers['X-Redmine-Switch-User'] = params.impersonate;
		...
		return d.promise.error(function(e){
			if ( e.message === 'ERROR_REQUEST' && retry.count < retry.maxTry ) {
				...
				return D.wait(Math.min(1 << retry.count, retry.maxDelay)).then(function(){
					retry.count++;
					params.retry = retry;
					options.headers['X-Redmine-Switch-User'] && (params.impersonate = options.headers['X-Redmine-Switch-User']);
